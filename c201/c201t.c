...
void List_Dispose(List *list)
{
    ...
    list->activeElement = NULL; // inicializace
    list->firstElement = NULL;
    list->currentLength = 0;
}
...
{
    ListElementPtr newElem = (ListElementPtr)malloc(sizeof(struct ListElement)); // vytvoří nový prvek a uloží adresu
    if (newElem == NULL)                                                         // kontrola
    {
        List_Error();
        exit(1);
    }
...
{
    ListElementPtr currentElem = list->firstElement;
    if (currentElem != NULL)
    {
        if (list->activeElement == currentElem)
        {
            list->activeElement = NULL;
        }
        list->firstElement = currentElem->nextElement;
        free(currentElem);
        list->currentLength--;
    }
}
...
{
    ListElementPtr activeElem = list->activeElement;
    if (activeElem != NULL)
    {
        ListElementPtr targetElem = activeElem->nextElement;
        if (targetElem != NULL)
        {
            activeElem->nextElement = targetElem->nextElement;
            free(targetElem);
            list->currentLength--;
        }
    }
...
void List_InsertAfter(List *list, int data)
{
    ...
    if (activeElem != NULL)
    {
        ...
        newElem->nextElement = activeElem->nextElement;
        newElem->data = data;
        activeElem->nextElement = newElem;
        // printf("He"); // funguje jen když je zde víc jak 2 písmena
        list->currentLength++;
    }
}
...
void List_Next(List *list)
{
    ...
    if (activeElem != NULL)
    {
        ListElementPtr nextElem = activeElem->nextElement;
        list->activeElement = nextElem;
    }
}
