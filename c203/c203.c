...
{
	if (queue == NULL) // kontrola zda parametr queue je platný ukazatel
	{
		Queue_Error(QERR_INIT);
		return;
	}
	...
	{
		queue->array[i] = '*'; // nastaví všechny prvky fronty na '*'
	}
	queue->firstIndex = 0;
	queue->freeIndex = 0; // nastaví oba indexi na první prvek ve frontě
}
...
{
	return ((index + 1) % QUEUE_SIZE); // vratí index o jedna větší,
									   // pomocí modula vrátí index v rozsahu fronty
}
...
{
	return (queue->freeIndex == queue->firstIndex) ? 1 : 0;
}
...
{
	if (Queue_IsEmpty(queue) == 1) // kontrola zda fronta není prázdná
	{
		Queue_Error(QERR_FRONT);
		return;
	}
	*dataPtr = queue->array[queue->firstIndex]; // nahraní hodnoty ze začátku fronty do místa *dataPtr
}
...
void Queue_Remove(Queue *queue)
{
	...
	queue->firstIndex = nextIndex(queue->firstIndex); // posunutí indexu (začatek fronty) o jedna dopředu v rámci rozsahu polí fronty
}
...
void Queue_Dequeue(Queue *queue, char *dataPtr)
{
	...
	Queue_Front(queue, dataPtr); // získá hodnotu ze začátku fronty
	Queue_Remove(queue);		 // posune začátek fronty
}
...
void Queue_Enqueue(Queue *queue, char data)
{
	...
	queue->array[queue->freeIndex] = data;			// vloží hodnotu z parametru data do prvního volného prvku fronty
	queue->freeIndex = nextIndex(queue->freeIndex); // posune index (první volný prvek) o jedna dál
}
